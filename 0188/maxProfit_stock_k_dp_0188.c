...
int maxProfit(int time, int* prices, int pricesSize)
{
    memset(g_dp, 0x0, sizeof(g_dp));
    if (time > pricesSize / 2) {
        return maxProfit_infi(prices, pricesSize);
    }
    if (time == 1) {
        return maxProfit_one_youhua(prices, pricesSize);
    }
    for (int i = 0; i < pricesSize; i++) {
    ...
    return g_dp[pricesSize - 1][time][0];
}
